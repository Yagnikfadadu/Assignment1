...
int main(){
   ...
   init_tile_config (&tile_data);
    ...
    //MY1 
    struct perf_event_attr pe,pe_cycle;
    long long count,count_cycle;
    int fd,fd_cycle;

    memset(&pe, 0, sizeof(struct perf_event_attr));
    memset(&pe_cycle, 0, sizeof(struct perf_event_attr));
    pe.type = PERF_TYPE_RAW;
    pe_cycle.type = PERF_TYPE_RAW;
    pe.size = sizeof(struct perf_event_attr);
    pe_cycle.size = sizeof(struct perf_event_attr);
    // config = (UMASK << 8) | EVENTSEL
    //config = (0x02 << 8) | 0xB7
    //  = 0x0200 | 0xB7
    //  = 0x02B7
 
 // EventSel = 0xB7, UMask = 0x02 => raw = (UMask << 8) | EventSel
    pe.config = (0x02 << 8) | 0xB7;   // = 0x2B7
    pe_cycle.config = 0x3C; 
    pe.disabled = 0;
    pe_cycle.disabled = 0;
    pe.exclude_kernel = 0;
    pe_cycle.exclude_kernel = 0;
    pe_cycle.exclude_hv = 0;
    pe.exclude_hv = 0;
...
//   print_buffer32(res, rows, colsb/4);
   ...
   //My1
    ...
    read(fd_cycle, &count_cycle, sizeof(long long));
